...
* 설명 : AVI 파일을 CAvi 객체로 불러온다.
* 
*************************************************************************/

BOOL CAvi::Open(LPCTSTR lpszPathName) //CString 타입임 - 파일오픈시 전체 경로_
{
	int i;
	HRESULT hr;
	...
	{ 
		...
		if( aviStreamInfo.fccType == streamtypeVIDEO )
		{
			...
			m_pVideoFrame = AVIStreamGetFrameOpen(m_pVideoStream, (LPBITMAPINFOHEADER)AVIGETFRAMEF_BESTDISPLAYFMT);
			if( !m_pVideoFrame )
			{
				Close();
				return FALSE;
...
/*************************************************************************
* 
* 이름 : GetFrame(int nFrame, CDib& dib)
* 인자 : nFrame - 현재 프레임 번호
*        dib    - 현재 프레임을 저장한 CDib 객체
* 반환 : 성공하면 TRUE, 실패하면 FALSE를 반환.
...
BOOL CAvi::GetFrame(int nFrame, CDib& dib)
{
	...
	else if( lpbi->bmiHeader.biBitCount == 32 )			// 2013-10-01 JMR
	{
		int i,j;	
		...
		for (i=0;i<m_nHeight;i++)		
		{
			...
			{				
				ptr[i][j].r = lpbi->bmiColors[(m_nHeight-i-1)*m_nWidth+j].rgbRed;
				ptr[i][j].g = lpbi->bmiColors[(m_nHeight-i-1)*m_nWidth+j].rgbGreen;
				ptr[i][j].b = lpbi->bmiColors[(m_nHeight-i-1)*m_nWidth+j].rgbBlue; 
			 }
		}		
	}
	else
